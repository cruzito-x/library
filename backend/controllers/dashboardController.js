...
exports.getTopSellers = (req, res) => {
  const period = req.query.period || 7;
  const selectTopSellers = "select l.idLibro, l.titulo, sum(dv.cantidad) as totalVendido from libros l join detalles_venta dv on l.idLibro = dv.idLibro join ventas v on dv.idVenta = v.idVenta where v.estado = 'completado' and v.fecha >= now() - interval ? day group by l.idLibro, l.titulo order by totalVendido desc limit 5;";
...
exports.getLastFiveBooks = (req, res) => {
  const selectLastBooks = "select *, e.existencia as stock from libros l inner join existencias e on e.idLibro = l.idLibro inner join genero g on g.idGenero = l.genero where (l.deleted_at is null and e.deleted_at is null and g.deleted_at is null and l.deleted_at is null) order by l.created_at desc limit 5;";
  
  db.query(selectLastBooks, (error, results) => {
      if (error) {
