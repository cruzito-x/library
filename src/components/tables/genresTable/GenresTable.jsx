...
const GenresTable = ({ genresData, refreshTable, setRefreshTable }) => {
  ...
  const handleGenreClick = (idGenero) => {
    fetch(`http://127.0.0.1:3001/genres/booksByGenre/${idGenero}`)
      .then((response) => {
        if (!response.ok) {
  ...
  const saveChanges = () => {
    form.validateFields().then((values) => {
      ...
      fetch(
        `http://127.0.0.1:3001/genres/updateGenre/${editedGenre.idGenero}`,
        {
          method: "put",
          headers: {
  ...
  const confirmDelete = (record) => {
    fetch(
      `http://127.0.0.1:3001/genres/deleteGenreUpdatedDeletedAt/${record.idGenero}`,
      {
        method: "delete",
      }
    )
