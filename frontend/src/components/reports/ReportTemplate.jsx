...
const ReportTemplate = ({ reportData, period }) => {
  ...
  useEffect(() => {
    if (reportData.length > 0 && !pdfGenerated) {
      ...
      doc.setFontSize(20);
      doc.text(`${
        period == 7 ? "Informe de ventas semanal" : period == 14 ? "Informe de ventas de los últimos 14 días" : period == 30 ? "Informe de ventas mensual" : period == 180 ? "Informe de ventas de los últimos 6 meses" : period == 365 ? "Informe de ventas anual" : ""}`, 105, 15, {
        align: "center",
      });
      ...
      doc.setFontSize(12);
      doc.text(`${ period == 7 ? "Durante la" : (period == 14 ? "Durante las" : (period == 30 ? "Durante el" : (period == 180 ? "Durante los meses" : (period == 365 ? "Durante el año" : ""))))} ${ (period == 7 || period == 14) ? reportData[1][0].week : (period == 30 ? reportData[1][0].month : (period == 180 ? reportData[1][0].semester : (period == 365 ? reportData[1][0].year : "")))}, hemos observado un desempeño notable en las ventas de libros. Se ha registrado un ${ reportData[5][0].porcentaje_incremento_decremento === ("incremento del [incremento infinito]%" || "decremento del [incremento infinito]%") ? "0% de incremento o decremento" : reportData[5][0].porcentaje_incremento_decremento } en comparación a ${ period == 7 ? "la semana anterior" : (period == 7 ? "las 2 semanas anteriores" : (period == 30 ? "el mes anterior" : (period == 180 ? "los últimos 6 meses" : (period == 365 ? "el último año" : "")))) }, lo que indica una tendencia ${ reportData[5][0].ventas_hoy < (reportData[5][0].ventas_hace_7_dias || reportData[5][0].ventas_hace_14_dias || reportData[5][0].ventas_hace_1_mes || reportData[5][0].ventas_hace_6_meses || reportData[5][0].ventas_hace_1_año) ? "negativa" : "positiva" } en el mercado de libros. Este informe detalla los aspectos clave de nuestras ventas, destacando los géneros más populares, los títulos más vendidos y otros datos relevantes para la toma de decisiones estratégicas.`,
        10,
        90,
        { maxWidth: 190 }
      );
