...
const StockTable = ({ stockData, refreshTable, setRefreshTable }) => {
  ...
  useEffect(() => {
    setLoading(true);
    fetch("http://192.168.0.3:3001/stock")
      .then((response) => {
        if (!response.ok) {
  ...
  const saveChanges = () => {
    form.validateFields().then((values) => {
      fetch(`http://192.168.0.3:3001/stock/updateStock/${editedStock.idLibro}`, {
        method: "put",
        headers: {
  ...
  const confirmDelete = (record) => {
    fetch(
      `http://192.168.0.3:3001/stock/deleteStockUpdatedDeletedAt/${record.idLibro}`,
      {
        method: "delete",
      }
    )
  ...
  const activateStock = (record) => {
    fetch(`http://192.168.0.3:3001/stock/activateStock/${record.idLibro}`, {
      method: "put",
    })
      .then((response) => {
        if (!response.ok) {
