...
const StockTable = ({ stockData, refreshTable, setRefreshTable }) => {
  ...
  const saveChanges = () => {
    form.validateFields().then((values) => {
      fetch(`http://127.0.0.1:3001/stock/updateStock/${editedStock.idLibro}`, {
        method: "put",
        headers: {
  ...
  const confirmDelete = (record) => {
    fetch(
      `http://127.0.0.1:3001/stock/deleteStockUpdatedDeletedAt/${record.idLibro}`,
      {
        method: "delete",
      }
    )
  ...
  const activateStock = (record) => {
    fetch(`http://127.0.0.1:3001/stock/activateStock/${record.idLibro}`, {
      method: "put",
    })
      .then((response) => {
        if (!response.ok) {
