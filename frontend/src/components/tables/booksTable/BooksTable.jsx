...
const BooksTable = ( { booksData, refreshTable, setRefreshTable } ) => {
  ...
  const saveChanges = () => {
    form.validateFields().then((values) => {
      fetch(`http://192.168.0.3:3001/books/updateBook/${editedBook.idLibro}`, {
        method: "put",
        headers: {
  ...
  const confirmDelete = (record) => {
    fetch(
      `http://192.168.0.3:3001/books/deleteBookUpdatedDeletedAt/${record.idLibro}`,
      {
        method: "delete",
      }
    )
          ...
          >
            {selectedRowData && (
              <>
                ...
                >
                  <Image
                    src={selectedRowData.portada === null ? "logo512.png" : `http://192.168.0.3:3001${selectedRowData.portada}`}
