...
const UsersTable = ({ usersData, refreshTable, setRefreshTable }) => {
  ...
  useEffect(() => {
    fetch("http://192.168.0.6:3001/users/")
      .then((response) => {
        if (!response.ok) {
  ...
  const saveChanges = () => {
    form.validateFields().then((values) => {
      ...
      fetch(`http://192.168.0.6:3001/users/updateUser/${editedUser.idUsuario}`, {
        method: "put",
        headers: {
  ...
  const confirmDelete = (record) => {
    fetch(
      `http://192.168.0.6:3001/users/deleteUserUpdatedDeletedAt/${record.idUsuario}`,
      {
        method: "delete",
      }
    )
